 ...
 * Supports standalone (desktop sidebar) and embedded (mobile tab) modes
 */
...
export const ChatPanel: React.FC<ChatPanelProps> = ({
  ...
  // ===== EMBEDDED MODE (Mobile tab content) =====
  if (embedded) {
    return (
      <div className="flex flex-col h-full">
          ...
          {messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`flex gap-2.5 ${isMe ? "flex-row-reverse" : ""}`}>
                {/* Avatar */}
                <div className={`w-7 h-7 rounded-full flex-shrink-0 flex items-center justify-center text-white text-[10px] font-bold bg-gradient-to-br ${
                  isMe ? "from-primary-500 to-purple-600" : "from-blue-500 to-indigo-600"
                }`}>
                  {msg.userId.substring(0, 1).toUpperCase()}
                </div>
                {/* Message bubble */}
                <div className={`max-w-[75%] ${isMe ? "bg-primary-600/20 border-primary-500/20" : "bg-dark-700/60 border-surface-glass-border"} border rounded-xl px-3 py-2`}>
                  {!isMe && (
                  ...
                  <span className="text-[9px] text-gray-600 mt-0.5 block text-right">
        ...
        {/* Input */}
        <form onSubmit={handleSubmit} className="flex items-center gap-2 px-4 py-3 border-t border-surface-glass-border bg-dark-900/60">
          <input
            type="text"
            value={input}
            ...
            maxLength={500}
            className="flex-1 bg-dark-800/80 border border-surface-glass-border text-white text-sm px-3 py-2.5 rounded-xl placeholder-gray-500 focus:border-primary-500/50 focus:ring-1 focus:ring-primary-500/20 outline-none"
          />
          <button
            type="submit"
            disabled={!input.trim()}
            className="w-9 h-9 flex items-center justify-center rounded-xl bg-primary-600 hover:bg-primary-500 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
          >
  ...
  // ===== STANDALONE MODE (Desktop sidebar) =====
  return (
